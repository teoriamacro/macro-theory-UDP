...
# podemos mostrar un histograma de los valores simulados a posteriori
...
# Una forma conveniente de calcular una densidad predictiva para cualquier priori es mediante simulación.
# Para obtener y  , primero simulamos, digamos, p??? desde g(p), y después simulamos y  desde la distribución binomial fB(~y|p???).
# Demostramos este enfoque de simulación para el beta(3.26,7.19) a priori.
...
# Guardamos las frecuancias y   en un vector frecuencias. Desoués convertimos las frecuencias a probabilidades dividiendo cada frecuencia por la suma y usando el comando plot para graficar la distribución predictiva
...
#Suponer que queremos resumir esta distribución predictiva por un intervalo que cubre al menos 90% de la probabilidad.
# La función discint es util para este proposito.
...
# La matriz dist contiene la distribución de probabilidad con las columnas ys y predprob.
# La función discint tiene dos entradas: La matriz dist y una cobertura de probabilidad covprob.
...
# De otra manera, sea ~y/20 denota la proporción de durmientes en la muestra futura.
# La probabilidad que esta proporción de muestra caiga en el intervalo [1/20,11/20] es de 91.8%.
# Como se esperaba, este intervalo es mucho mas ancho que un intervalo de probabilidad 91.8% por la proporción de la población p.
# En la predicción de una futura proporción de muestra, hay 2 fuentes de incertidumbre, la incertidumbre en el valor de p y la incertidumbre binomial en el valor ~y, y el intervalo predictivo es relativamente largo desde que incorpora ambos tipos de incertidumbre. 
